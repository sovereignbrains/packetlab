...
MOD_USERS='{"name":"%name%","uuid":"%uuid%","flow":"xtls-rprx-vision"}'
...
mod_status() {
  ...
  pl_meta_has reality_pbk || { printf broken; return; }
  printf up
}
...
mod_link() {  # mod_link <формат> [пользователь]
  local name="REALITY" u uuid
  u=$(pl_link_user "${2:-}"); uuid=$(pl_cred "$u" "$MOD_ID" uuid) || return 1
  case "$1" in
    clash) cat <<YAML
- name: ${name}
  type: vless
...
JSON
    ...
    uri) printf 'vless://%s@%s:443?security=reality&sni=%s&fp=chrome&pbk=%s&sid=%s&flow=xtls-rprx-vision&type=tcp#%s\n' \
      "$uuid" "$PL_HOST" "$(pl_meta_get ${MOD_ID}_sni)" \
