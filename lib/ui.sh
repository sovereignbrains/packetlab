...
# Windows-клиенты SSH не включают на pty IUTF8: русская буква для ядра — два
# символа, и лишний Backspace съедает приглашение. Включаем сами.
[ -t 1 ] && stty iutf8 2>/dev/null </dev/tty
...
# Экран меню: перед перерисовкой чистим видимую часть, иначе после каждого
# ответа под старым меню печаталось такое же новое. Вывод по-прежнему
# построчный; вне терминала (пайп, лог) ничего не чистим.
ui_clear() { [ -t 1 ] && printf '\033[H\033[2J'; }
ui_screen() {
  ui_clear
  ui_head "$@"
}

# ui_qr <текст> — QR прямо в терминал полублоками UTF-8 (два ряда модулей на
# строку). В цвете (ANSIUTF8) код сам задаёт чёрное и белое и сканируется
# при любой теме терминала; без цвета — вариант под тёмный фон. Ширина —
# число модулей плюс поля: в узкое окно не влезет, и перенос строк сломает
# код, поэтому предупреждаем.
ui_qr() {
  command -v qrencode >/dev/null 2>&1 || { ui_warn "qrencode не установлен — apt install qrencode"; return 1; }
  local qr first cols type=UTF8
  [ -n "$C_RST" ] && type=ANSIUTF8
  qr=$(qrencode -t "$type" -m 2 -o - "$1" 2>/dev/null) || { ui_err "qrencode не смог закодировать строку"; return 1; }
  first=$(printf '%s\n' "$qr" | head -1 | sed -E $'s/\033\\[[0-9;]*m//g')
  cols=$(tput cols 2>/dev/null || echo 80)
  [ $(( ${#first} + 2 )) -gt "$cols" ] \
    && ui_warn "QR шириной $(( ${#first} + 2 )) колонок, а окно — $cols: разверни окно или уменьши шрифт"
  printf '%s\n' "$qr" | sed 's/^/  /'
}
